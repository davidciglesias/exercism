...
const areSequential = (name1: string, name2: string): boolean => {
    ...
    const alphaDiff =
        (alpha2.charCodeAt(0) - alpha1.charCodeAt(0)) * 26 + (alpha2.charCodeAt(1) - alpha1.charCodeAt(1));
...
describe("Robot", () => {
    ...
    it("forcing the same name twice should throw ", () => {
        const forcedName = "AA0101";
        const robot1 = new RobotName(forcedName);
        expect(robot1.name).toEqual(forcedName);
        expect(() => new RobotName(forcedName)).toThrowError("Unable to force name as it has been used already.");
    });
});
